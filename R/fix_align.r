...
fix_align <- function(start_pts,
  ...
  if (summary_file) {
    out_file_name <-
      paste(fa_dir, '/', gsub('\\s+', '_', format(Sys.time(), format = '%B %d %Y')), '.fas', sep =
              '')
    out_file <- file(out_file_name, 'w')
    cat(
      'File Trial Slope VOffset SD Fit n_total_fix n_keep n_oob n_amb n_den n_nit n_part\n',
      file = out_file
    )
  ...
  for (i in 1:n_files) {
    ...
    asc_data <- read_fix_report(asc_files)
    ...
    for (t in 1:n_trials) {
      ...
      fix_data_trial <-
        mark_out_of_bounds(fix_data_trial, xy_bounds, t, n_trials)
  ...
  function(line_ret,
    ...
    if (show_image) {
      ...
      image_file_name_trial <-
        paste(substr(sub_file, 1, nchar(sub_file) - 4), '_', t, '.png', sep =
                '') # modified: bing -----------------------

      if (file.exists(image_file_name_trial)) {
        # Load the image
        t_image <- readPNG(image_file_name_trial)

      } else {
        # Get the subject specific image
        image_file_name_sub <-
          paste(substr(sub_file, 1, nchar(sub_file) - 4),  '.png', sep = '') # modified: bing -----------------

        if (file.exists(image_file_name_sub)) {
          # Load the image
          t_image <- readPNG(image_file_name_sub)
